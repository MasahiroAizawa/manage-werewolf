...
class @ManageWolf
  ...
  loadPlayer: =>
    if is_iOS()
      player_data = window.returnValue
      if player_data?
...
callLoader = (args) ->
  if is_iOS()
    window.loader_mode = args
    image_loader = new LoadImageDialog()

    $("div#load-modal").removeClass "hide"
    $("#mask").removeClass "hide"
  else
    return window.showModalDialog('/load_image', args, "status:off;")
